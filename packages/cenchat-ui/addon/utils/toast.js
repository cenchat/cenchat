...
function setupButtonElement(options) {
  if (Object.prototype.hasOwnProperty.call(options, 'text')) {
    ...
    buttonElement.classList.add('cen-button');
    buttonElement.classList.add('cen-toast__button');
...
export default async function toast(text, duration = 5000, buttonOptions = {}) {
  toastElement = document.querySelector('.cen-toast');

  if (toastElement) {
    const toastTextElement = document.querySelector('.cen-toast__text');
